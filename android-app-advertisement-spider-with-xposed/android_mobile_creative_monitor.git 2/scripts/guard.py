...
def get_need(line):
        try:
                cmd = line.cmdline
                if(line.name == 'adb'):
                    if(len(cmd) >=2 and (cmd[1] == '-s' or cmd[1] =='-P')):
                        if(len(cmd) >= 4):
                            pass
                        #if(cmd[3] == 'fork-server'):
                                                #return False
                            return True
                if(line.name == pythonName):
...
def is_illegal(target_list):
        ...
        t_cmds[3] = ['python','','ignore','ignore','ignore']
...
def start(num):
    ...
    str1 = "python  " + path.getPath("test.py") + "  " + num #+ " > /Users/yuhaiqiang/Desktop/log/test.log"
...
def killGuard():
        pro_list = is_exist_in_process_list(pythonName)
        print "杀死其他监控进程"
